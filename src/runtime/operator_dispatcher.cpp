...
#define BINARY(opcode, type1, type2) binary_dispatch_table_[static_cast<size_t>(OpCode::opcode)][static_cast<size_t>(ValueType::type1)][static_cast<size_t>(ValueType::type2)] = [](param_t lhs, param_t rhs) -> return_t
...
OperatorDispatcher::OperatorDispatcher(meow::memory::MemoryManager* heap) noexcept : heap_(heap) {
    ...
    binary_dispatch_table_[+ADD][+Int][+Int] = [](param_t lhs, param_t rhs) -> return_t {
        return Value(lhs.as_int() + rhs.as_int());
    };

    BINARY(ADD, Float, Float) {
        return Value(lhs.as_float() + lhs.as_float());
    };
