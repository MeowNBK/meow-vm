...
bool file_exists(const std::filesystem::path& p) noexcept {
    try { return std::filesystem::exists(p); } catch (...) { return false; }
}
...
std::string expand_token(const std::string& raw, const std::string& token, const std::filesystem::path& replacement) noexcept {
    ...
    while (true) {
        ...
        if (p == std::string::npos) { out.append(raw.substr(pos)); break; }
    ...
    try {
        ...
        if (!ext.empty()) {
            ...
            for (const auto& f : forbidden_extensions) {
                if (ext_l == to_lower_copy(f)) return ""; // caller indicated do-not-treat-as-native
            }
        ...
        // If no extension on module_path and candidate_extensions provided, try each appended ext.
        ...
        if (extra_relative_search) {
            ...
            if (importer == entry_path) base_dir = std::filesystem::path(entry_path);
            else base_dir = std::filesystem::path(importer).parent_path();
