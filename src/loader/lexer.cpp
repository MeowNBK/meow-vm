...
static const std::unordered_map<std::string_view, TokenType> OPCODE_LEX_MAP = []{
    ...
    map["GET_EXPORT"] = TokenType::OPCODE; /*GET_MODULE_EXPORT is removed?*/ map["IMPORT_ALL"] = TokenType::OPCODE;
    return map;
}();

static constexpr std::array<std::string_view, static_cast<size_t>(TokenType::TOTAL_TOKENS)> TOKEN_MAP = {
        // Directives
        "DIR_FUNC",       // .func
        "DIR_ENDFUNC",    // .endfunc
...
Token Lexer::make_token(TokenType type) const {
    return Token{
        src_.substr(token_start_pos_, pos_ - token_start_pos_),
        type,
        token_start_line_,
        token_start_col_
    };
...
void Lexer::skip_whitespace() noexcept {
    while (std::isspace(curr_)) advance();
}
...
bool Lexer::is_identifier_start(unsigned char c) {
...
Token Lexer::scan_identifier() {
    ...
    if (starts_with_dot) {
        ...
        if (!is_identifier_start(curr_) || curr_ == '@') {
             ...
             return make_token(TokenType::UNKNOWN); // Lỗi: '.' không theo sau bởi identifier hợp lệ
        }
    }

    while (std::isalnum(curr_) || curr_ == '_') advance();
...
Token Lexer::scan_number() {
    ...
    if (curr_ == '-') {
        maybe_negative = true;
        advance();
        // Nếu sau dấu '-' không phải là số -> lỗi hoặc là toán tử (không xử lý ở đây)
    ...
    if (!maybe_negative && curr_ == '0' && pos_ + 1 < src_.size()) {
        ...
        if (next == 'x' || next == 'X') { // Hex
            advance(); advance(); // Bỏ qua "0x"
            while (std::isxdigit(curr_)) advance();
            return make_token(TokenType::NUMBER_INT);
        } else if (next == 'b' || next == 'B') { // Binary
            advance(); advance(); // Bỏ qua "0b"
            while (curr_ == '0' || curr_ == '1') advance();
            return make_token(TokenType::NUMBER_INT);
        } else if (next == 'o' || next == 'O') { // Octal
             advance(); advance(); // Bỏ qua "0o"
             while (curr_ >= '0' && curr_ <= '7') advance();
             return make_token(TokenType::NUMBER_INT);
        }
        // Nếu chỉ là '0' hoặc '0' theo sau bởi ký tự khác -> số thập phân bắt đầu bằng 0
    }
    ...
    if (is_float && (curr_ == 'e' || curr_ == 'E')) {
        ...
        if (!std::isdigit(curr_)) {
            // Lỗi: Thiếu số sau 'e'/'E'
             ...
             if (curr_ == '+' || curr_ == '-') pos_--; // Lùi thêm nếu có dấu
