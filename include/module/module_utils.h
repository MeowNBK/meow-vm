...
std::filesystem::path get_executable_dir() noexcept; // absolute dir of current executable (fallback: current_path)
...
// config_filename: filename to look for next to exe_dir (caller decides, e.g. "meow-root" or "")
// token: expansion token (caller decides, e.g. "$ORIGIN" or "")
// treat_bin_as_parent: if exe_dir filename == "bin" treat exe_dir.parent_path() as fallback root
// exe_dir_provider: optionally injected (useful for tests)
// Returns absolute normalized path. Thread-safe (internally caches results keyed by inputs).
std::filesystem::path detect_root_cached(const std::string& config_filename,
                                         const std::string& token,
                                         bool treat_bin_as_parent = true,
                                         std::function<std::filesystem::path()> exe_dir_provider = get_executable_dir) noexcept;

// Build a set of common search roots from a detected root (caller can still pass any roots).
// This is only a helper to produce typical patterns (root, root/lib, root/stdlib, root/bin/stdlib, root/bin).
...
// entry_path: entry path string used as special-case base (caller-defined meaning)
// forbidden_extensions: extensions which indicate "language files" (do not try native), lowercase expected but function is case-insensitive
// candidate_extensions: list of extensions to try if module_path has no extension (order matters)
// search_roots: additional absolute directories to search (in order)
// extra_relative_search: attempt base_dir(importer)/module candidates as last resort
// Returns absolute normalized existing path, or empty string if not found.
...
// --- platform errors ---
std::string platform_last_error() noexcept; // human-friendly last error from OS loader (may be empty)
