 ...
 * Example usage:
 *  @code
 *  using V1 = meow::utils::VariantFast<int, float, char>;
 *  using V2 = meow::utils::VariantFast<V1, double, long>;
 *
 *  V2 v = 3.14;
 *  if (v.holds<double>()) {
    ...
    static void copy_assign_impl(void* dst, const void* src) {
        if constexpr (std::is_copy_assignable<T>::value) {
            *reinterpret_cast<T*>(dst) = *reinterpret_cast<const T*>(src);
            ...
            // fallback replace (destroy + copy-construct)
...
public:
    ...
    // copy ctor / move ctor using ops tables
    ...
    // copy assignment (strong via copy-and-swap)
    Variant& operator=(const Variant& other) {
        if (this == &other) return *this;
        if (other.index_ == npos) { destroy_current(); index_ = npos; return *this; }
        if (index_ == other.index_) {
            // same index -> copy-assign via table
            ops_table().copy_assign_table[index_](storage_ptr(), other.storage_ptr());
            return *this;
        }
        Variant tmp(other);
        swap(tmp);
    ...
    Variant& operator=(T&& v) noexcept(std::is_nothrow_constructible<U, T&&>::value) {
        ...
        if (index_ == idx) {
            ops_table().copy_assign_table[idx](storage_ptr(), &v);
        } else {
            destroy_current();
            construct_by_type<U>(std::forward<T>(v));
            index_ = idx;
    ...
    // holds supports nested Variant as before
    template <typename T>
    ...
    void swap(Variant& other) noexcept {
        ...
        // different indices -> move via temporaries (fast move tables)
        Variant tmp_self;
        Variant tmp_other;
...
private:
    // underlying ops_table for these concrete flat_list types
    using ops_t = ops<typename nth_type<0, flat_list>::type // trick to expand pack below
                      // we need to expand full list into ops<...>, but can't in one shot here.
                      >;
    // NOTE: the above is only a placeholder; we'll create an ops<AllTypes...> alias below using a helper:
    ...
    static constexpr full_ops& ops_table() { return *static_cast<full_ops*>(nullptr); } // we only use the static inline tables in full_ops, so this is never deref'd
