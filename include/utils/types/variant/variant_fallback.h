...
template <typename...> class FallbackVariant;
template <typename T> struct is_variant : std::false_type {};
template <typename... Us> struct is_variant<FallbackVariant<Us...>> : std::true_type {};
...
// visitor_returns: ensure a visitor returns the same type for all alternatives (mimic std::variant checks)
...
public:
    ...
    template <typename T>
    static constexpr std::size_t index_of() noexcept { return detail::type_list_index_of<std::decay_t<T>, flat_list>::value; }

    // get / get_if (typed)
    template <typename T>
    const T& get() const noexcept { return unsafe_get_unchecked<T>(); }
    template <typename T>
    T& get() noexcept { return unsafe_get_unchecked<T>(); }
    ...
    template <typename T>
    std::enable_if_t<is_variant<std::decay_t<T>>::value, std::decay_t<T>>
    get() const noexcept {
    ...
    decltype(auto) visit(Visitor&& vis) {
        ...
        static_assert(visitor_returns<Visitor, flat_list, 0>::value,
            "FallbackVariant::visit: visitor must return same type for all alternatives (or use void)");
    ...
    decltype(auto) visit(Visitor&& vis) const {
        ...
        static_assert(visitor_returns<Visitor, flat_list, 1>::value,
            "FallbackVariant::visit(const): visitor must return same type for all alternatives (or use void)");
    ...
    void swap(FallbackVariant& other) noexcept {
        ...
        if (index_ != npos) { tmp_self.move_from_index(static_cast<std::size_t>(index_), storage_ptr()); tmp_self.index_ = index_; }
        if (other.index_ != npos) { tmp_other.move_from_index(static_cast<std::size_t>(other.index_), other.storage_ptr()); tmp_other.index_ = other.index_; }
        destroy_current(); other.destroy_current();
        if (tmp_other.index_ != npos) { move_from_index(static_cast<std::size_t>(tmp_other.index_), tmp_other.storage_ptr()); index_ = tmp_other.index_; }
        if (tmp_self.index_  != npos) { other.move_from_index(static_cast<std::size_t>(tmp_self.index_), tmp_self.storage_ptr()); other.index_ = tmp_self.index_; }
    }
    ...
    static FallbackVariant from_raw_bits(uint64_t bits) noexcept {
        ...
        if (idx == 0xFFu) v.index_ = npos;
        else v.index_ = static_cast<index_t>(idx);
...
private:
    ...
    template <> struct max_sizeof<detail::type_list<>> : std::integral_constant<std::size_t, 0> {};
    ...
    template <> struct max_alignof<detail::type_list<>> : std::integral_constant<std::size_t, 1> {};
    template <typename H, typename... Ts>
    ...
    void swap_same_index_impl(index_t idx, FallbackVariant& other) {
        if constexpr (I < alternatives_count) {
            ...
            if (idx == static_cast<index_t>(I)) { using std::swap; swap(*reinterpret_cast<T*>(storage_ptr()), *reinterpret_cast<T*>(other.storage_ptr())); return; }
    ...
    bool holds_any_of_impl() const noexcept {
        if constexpr (I >= detail::type_list_length<InnerList>::value) return false;
        else {
