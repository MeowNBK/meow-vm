...
// variant_utils.h
// Metaprogramming utilities to flatten nested meow::variant<...> into a unique typelist.
// Minimal, efficient, and safe for header-only inclusion.

#include <cstddef>
#include <type_traits>
#include <utility>

namespace meow { template <typename...> class variant; } // forward-declare meow::variant
...
template <typename T> struct is_meow_variant : std::false_type {};
template <typename... Inner> struct is_meow_variant<meow::variant<Inner...>> : std::true_type {};

// variant_inner_list: if T is meow::variant<...> -> type_list<Inner...>, otherwise type_list<T>
template <typename T> struct variant_inner_list { using type = type_list<T>; };
template <typename... Inner> struct variant_inner_list<meow::variant<Inner...>> { using type = type_list<Inner...>; };

// ---------------- flatten_list_implement ----------------
// Accepts a sequence of types (which may be: normal types, type_list<...>, or meow::variant<...>)
// and produces a type_list<> containing unique types in order of first appearance.
...
private:
    ...
    template <typename... Inner> struct head_to_list<meow::variant<Inner...>> { using type = type_list<Inner...>; };
